    ...
    async initializeFromMemory(memory, logger, setPersonalityMode) {
        try {
            ...
            logger.info(
                ...
                `Memuat ${Object.keys(this.loadedLongTermMemory).length} preferensi dari memori jangka panjang.`,
            );

            this.isNgambekMode = (await memory.getPreference('isNgambekMode')) || false;
            this.lastInteractionTimestamp = (await memory.getPreference('lastInteractionTimestamp')) || null;
            this.dailyChatCounts = (await memory.getPreference('dailyChatCounts')) || {};
            logger.info(
                {
