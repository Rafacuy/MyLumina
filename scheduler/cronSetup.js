 ...
 */
const setupCronJobs = (bot, updateTimeBasedModes, checkNgambekStatus, USER_NAME, Sentry) => {
    const configuredChatId = config.TARGET_CHAT_ID;
    if (!configuredChatId) {
        logger.warn('⚠️ TARGET_CHAT_ID not found in config.js. Scheduled messages will NOT be sent.');
        return;
    }

    logger.info(`📬 Scheduled messages will be sent to chat ID: ${configuredChatId}`);
    ...
    if (isFeatureEnabled('ENABLE_WEATHER_REMINDER')) {
        schedule.scheduleJob({ rule: '33 14 * * *', tz: 'Asia/Jakarta' }, async () => {
            try {
    ...
    schedule.scheduleJob({ rule: '0 0 1 */2 *', tz: 'Asia/Jakarta' }, async () => {
    ...
    schedule.scheduleJob({ rule: '0 */7 * * *' }, async () => {
        logger.info({ event: 'relation_status_check_scheduled' }, 'Running scheduled relationship status check...');
        try {
    ...
    if (isFeatureEnabled('ENABLE_SONGS_NOTIFIER')) {
        ...
        logger.info({ event: 'songs_notifier_disabled' }, 'FF-CHECK: Song notifier job is disabled by feature flag.');
    }

    // Daily news and summary every morning at 8 AM
    // FF-CHECK: This job is guarded by its feature flag.
    if (isFeatureEnabled('ENABLE_DAILY_NEWS')) {
        schedule.scheduleJob({ rule: '0 8 * * *', tz: 'Asia/Jakarta' }, async () => {
            logger.info({ event: 'daily_news_send_scheduled' }, '[Core] Running scheduled daily news delivery...');
            try {
        ...
        logger.info({ event: 'daily_news_disabled' }, 'FF-CHECK: Daily news job is disabled by feature flag.');
    }
    ...
    schedule.scheduleJob({ rule: '0 * * * *', tz: 'Asia/Jakarta' }, async () => {
        logger.info({ event: 'update_chat_summary_start' }, '[Core] Updating chat summary...');
        try {
            ...
            if (summary) {
                ...
                logger.info({ event: 'update_chat_summary_success' }, '[Core] New chat summary created successfully.');
            } else {
    ...
    if (isFeatureEnabled('ENABLE_NGAMBEK_MODE')) {
        schedule.scheduleJob({ rule: '0 0 * * *', tz: 'Asia/Jakarta' }, async () => {
            logger.info({ event: 'sulk_status_check_scheduled' }, '[Sulk System] Checking sulk status...');
            try {
        ...
        logger.info({ event: 'sulk_mode_disabled' }, 'FF-CHECK: Sulk mode job is disabled by feature flag.');
    }
    ...
    if (isFeatureEnabled('ENABLE_HOLIDAYS_REMINDER')) {
        if (config.calendarificApiKey) {
            schedule.scheduleJob({ rule: '0 7 * * *', tz: 'Asia/Jakarta' }, async () => {
                try {
                    await holidaysModule.checkAndNotifyDailyHolidays(config.calendarificApiKey, 'ID', (message) =>
                        sendMessage(configuredChatId, message),
                    );
                    logger.info({ event: 'daily_holiday_check_scheduled' }, 'Daily holiday check performed.');
                } catch (error) {
                    logger.error(
                        {
            ...
            logger.warn('[Core] Calendarific API Key not found. Holiday check is disabled.');
        }
    } else {
        logger.info(
