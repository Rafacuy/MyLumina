/**
 * telegramClient.js - Telegram Bot API Client
 *
 * This module provides a unified interface for all Telegram Bot API operations.
 * It wraps the grammY framework with additional error handling, retry logic,
 * and logging to ensure reliable message delivery.
 *
 * Features:
 ...
 *
 * Architecture:
 * The TelegramClient uses a singleton pattern to maintain a single Bot instance
 * throughout the application lifecycle. It provides methods for:
 * - Sending text messages
 * - Sending photos and documents
 * - Getting file information
 * - Managing webhook/polling
 *
 * Configuration:
 * - Timeout: 30 seconds for API requests
 * - API root: Configurable via TELEGRAM_API_ROOT env var (for proxies)
 * - Retry logic: Built-in retry for network errors
 *
 * @module utils/telegramClient
 * @requires grammy
 ...
 *
 * This class wraps the grammY Bot instance and adds:
...
class TelegramClient {
    ...
    isTemporaryNetworkError(error) {
        ...
        return (
            tempCodes.includes(code) ||
            tempSnippets.some((sn) => message.includes(sn)) ||
            (!!error?.error_code && error.error_code >= 500)
        );
    }
    ...
    async sendChatAction(chatId, action = 'typing') {
        try {
            ...
            logger.debug({ event: 'chat_action_sent', chatId, action }, '[TelegramClient] Chat action sent');
        } catch (error) {
            logger.warn(
    ...
    async downloadFile(downloadUrl, targetPath) {
        try {
            ...
            logger.info({ event: 'file_downloaded', targetPath }, '[TelegramClient] File downloaded successfully');
    ...
    async stop() {
        try {
            await this.bot.stop();
            logger.info({ event: 'telegram_client_stopped' }, '[TelegramClient] Telegram client stopped gracefully');
        } catch (error) {
            logger.warn(
    ...
    async answerInlineQuery(inlineQueryId, results, options = {}) {
        try {
            ...
            logger.debug({ event: 'inline_query_answered', inlineQueryId }, '[TelegramClient] Inline query answered');
        } catch (error) {
            logger.warn(
