...
const setReminder = async (botInstance, chatId, timeString, message, userName) => {
    ...
    try {
        ...
        const formattedDate =
            formatJakartaDateTime(reminderTime).split(',')[0] +
            ', ' +
            formatJakartaDateTime(reminderTime).split(',')[1];
 ...
 * @param {object} botInstance - The Telegram Bot API instance.
 */
const rescheduleReminders = async (_botInstance) => {
    try {
            ...
            if (reminderTime.isValid() && reminderTime.isAfter(now)) {
                ...
                schedule.scheduleJob(reminderTime.toDate(), async () => {
                    try {
                        sendMessage(
                            reminder.chatId,
                            `🔔 Pengingat untuk Tuan ${reminder.userName || 'Pengguna'}:\n${reminder.content}`,
                        );
                        await memory.deleteLTMMemory(reminder.key);
                    } catch (scheduleError) {
                        console.error(
                            'Error in rescheduled reminder task:',
...
async function performSearch(query, userName, requestChatId, aiSummarizer) {
    ...
    try {
            ...
            if (typeof aiSummarizer === 'function' && contentToSummarize.trim() !== '') {
                ...
                try {
                    const summarizationPrompt =
                        'Sebagai Lumina, asisten AI yang cerdas dan sedikit tsundere, ' +
                        'ringkaskan dengan gaya khasmu informasi berikut yang ditemukan ' +
                        `untuk Tuan ${userName} terkait pencarian "${query}". ` +
                        `Buat ringkasan yang informatif namun tetap singkat dan menarik:\n\n${contentToSummarize}`;
                    const summary = await aiSummarizer(summarizationPrompt, requestChatId);
                    if (
                        summary &&
                        !summary.toLowerCase().includes('maaf') &&
                        !summary.toLowerCase().includes('zzz') &&
                        !summary.toLowerCase().includes('mohon bersabar') &&
                        summary.length > 15
                    ) {
                        resultText += `${summary}\n\n`;
                    } else {
                    ...
                    console.error(
                        'Error during summarization with Lumina:',
                        summarizationError.message,
                        summarizationError.stack,
                    );
        ...
        console.error(
            'Error fetching from Google Custom Search API:',
...
const getHelpMessage = (userName) => {
    try {
        return (
            `Daftar perintah Lumina untuk Tuan ${userName || 'Pengguna'}:\n\n` +
