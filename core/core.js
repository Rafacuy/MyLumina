/**
 * core.js - The beating heart of MyLumina
 *
 * This module orchestrates the entire bot lifecycle, from initialization to message processing.
 * it coordinates all the different parts
 * (handlers, modules, schedulers) to create a harmonious conversational experience.
 *
 * Key responsibilities:
 * - Bot initialization and dependency injection
 * - Message routing and command handling (optimized with O(1) lookups)
 * - Real-time state management (moods, relationships, ngambek mode)
 * - Integration with AI response generation
 * - Document, image, and location message processing
 * - Long-term memory processing for personalized conversations
 *
 * The code follows a structured flow: initialize -> optimize -> listen -> respond.
 * We use a Mutex for thread-safe state updates and feature flags for modular control.
 *
 * @module core/core
 * @author Arash
 * @version 1.2.3
...
// For O(1) lookup of prefixed commands (e.g., /help).
const commandMap = new Map(); 
// For iterating over more complex regex patterns.
const regexHandlers = [];
...
// Store manageCache for access from aiResponseGenerator
...
/**
 * Updates the user's interaction status with thread-safe operations.
 *
 * Every time a user sends a message, this function tracks it to maintain
 * accurate interaction statistics. It uses a Mutex to prevent race conditions
 * when multiple messages arrive simultaneously (imagine two people texting
 * at the exact same millisecond).
 *
 * The function tracks:
 * - Last interaction timestamp (for calculating idle time)
 * - Daily chat counts (for ngambek mode logic and relationship tracking)
 *
 * Data is persisted to LokiJS for durability across restarts.
 *
 * @param {string|number} [chatId=null] - The chat ID where interaction occurred, for logging context
 * @returns {Promise<void>}
 * @example
 * // Called automatically when user sends any message
 * await updateInteractionStatus(msg.chat.id);
 */
const updateInteractionStatus = async (chatId = null) => {
    ...
    await interactionMutex.runExclusive(async () => {
        try {
            ...
            globalState.dailyChatCounts = loadedCounts && typeof loadedCounts === 'object' ? loadedCounts : {};
            ...
            await memory.savePreference('lastInteractionTimestamp', globalState.lastInteractionTimestamp);
...
/**
 * Manages Lumina's "ngambek" (sulking) mode based on user interaction patterns.
 *
 * Ngambek is a core personality feature - if the user ignores Lumina for too long
 * (NGAMBEK_DURATION_DAYS), she'll enter a sulking state with a jealous mood.
 * To win her back, the user must actively chat for END_NGAMBEK_INTERACTION_DAYS
 * with at least MIN_CHATS_PER_DAY_TO_END_NGAMBEK messages per day.
 *
 * The logic uses a mathematical approach with Array.every() to check if all
 * required days have sufficient interaction, rather than manual loops.
 *
 * Edge cases handled:
 * - Feature flag disabled: Automatically clears ngambek state
 * - First-time users: Won't trigger ngambek (no last interaction date)
 * - Already in ngambek mode: Monitors for recovery
 *
 * @param {string|number} chatId - The chat ID to send mood change notifications to
 * @returns {Promise<void>}
 * @throws {Error} When state persistence fails (caught and logged)
 * @example
 * // Run periodically to check ngambek status
 * await checkNgambekStatus(config.TARGET_CHAT_ID);
 */
const checkNgambekStatus = async (chatId) => {
    ...
    if (!globalState.isNgambekMode && lastInteractionDate) {
        ...
        if (diffDays >= NGAMBEK_DURATION_DAYS) {
            ...
            sendMessage(
                chatId,
                'Hmph! Kamu kemana aja?! Lumina sekarang ngambek karena kamu tidak ' +
                `mendengarkan Lumina selama ${diffDays} hari! 😒`,
            );
        }
    }
    ...
    if (globalState.isNgambekMode) {
        // Generate an array of date strings for
        // the past N days.
        ...
        const hasSufficientInteraction = checkDates.every(
            (dateStr) => (globalState.dailyChatCounts[dateStr] || 0) >= MIN_CHATS_PER_DAY_TO_END_NGAMBEK,
        );
        ...
        if (hasSufficientInteraction) {
            ...
            await memory.savePreference('dailyChatCounts', globalState.dailyChatCounts);
            logger.info({ event: 'ngambek_mode_deactivated' }, '[Ngambek System] Lumina is no longer sulking!');
            sendMessage(
                chatId,
...
/**
 * Determines if a string consists entirely of emojis.
 *
 * Users sometimes send single emojis or emoji-only messages. Rather than
 * processing these through the AI system (which wastes tokens and adds latency),
 * we detect them early and skip AI response generation.
 *
 * This uses a comprehensive Unicode regex that matches:
 * - Emoji presentation characters (😀, 🎉, etc.)
 * - Emoji with variation selectors (❤️ vs ❤)
 * - Emoji modifiers (skin tones like 👍🏽)
 * - Emoji components (for complex sequences)
 *
 * @param {string} str - The input string to analyze
 * @returns {boolean} True if string contains only emojis, false otherwise
 * @example
 * //returns true
 * isOnlyEmojis('😀');       
 * //returns false 
...
/**
 * Determines if a string consists entirely of numeric digits.
 *
 * Single numbers (like "1" or "42") don't carry enough semantic meaning
 * for meaningful AI conversation. This filter prevents wasting AI calls
 * on numeric-only inputs that can't be properly contextualized.
 *
 * Note: This intentionally does NOT match negative numbers or decimals,
 * as those might be coordinates, temperatures, etc. that need context.
 *
 * @param {string} str - The input string to analyze
 * @returns {boolean} True if string contains only digits 0-9, false otherwise
 * @example
 * //returns false
 * isOnlyNumbers('42');     
 * //returns false (contains decimal)  
 * isOnlyNumbers('3.14');   
 * //returns false
 * isOnlyNumbers('hello');    
 */
...
/**
 * Analyzes user messages to extract and store long-term preferences.
 *
 * This is the entry point for Lumina's learning system. When users share
 * personal information ("I love chocolate", "I'm allergic to peanuts"),
 * this function detects those preferences and stores them for future
 * personalized conversations.
 *
 * The process:
 * 1. Feature flag check (ENABLE_LTM must be enabled)
 * 2. Length validation (ignore very short messages)
 * 3. AI analysis to identify preference-worthy content
 * 4. Storage via LTM processor if preferences found
 *
 * Errors are caught and logged but don't interrupt the main conversation flow
 * - we don't want a learning error to break the chat experience.
 *
 * @param {string} text - The user's message text to analyze
 * @returns {Promise<void>}
 * @example
 * // Called for every text message
 * await analyzeAndSavePreferences("My favorite color is blue");
 * // If analysis detects a preference, it's saved to memory
 */
const analyzeAndSavePreferences = async (text) => {
    if (!isFeatureEnabled('ENABLE_LTM')) {
        return;
    }
    ...
    try {
        ...
        logger.error({ event: 'ltm_processing_error', error: error.message }, 'Error in LTM processing pipeline');
        Sentry.captureException(error);
    }
};

/**
 * Configures the main message event listener for the Telegram bot.
 *
 * This is where the magic happens - every incoming message flows through here.
 * The function sets up a comprehensive handler that categorizes messages
 * by type and routes them to the appropriate processing logic.
 *
 * Message handling flow:
 * 1. **Location messages** → Save location and fetch weather
 * 2. **Document messages** → Extract text and summarize with AI
 * 3. **Image messages** → Analyze with Vision AI and respond contextually
 * 4. **Text messages** → Check commands → Check regex patterns → AI response
 *
 * Each handler is wrapped in error handling to ensure one failed message
 * type doesn't crash the entire bot. Messages are saved to memory for
 * conversation continuity.
 *
 * Optimization: Command handlers are pre-categorized during initialization
 * for O(1) lookup of prefixed commands (/help, /start, etc.)
 *
 * @param {object} bot - The grammY Bot instance with Telegram API connection
 * @returns {void}
 * @example
 * // Called once during bot initialization
 * setupMessageListener(bot);
 * // Now bot.on('message', ...) is listening for all incoming messages
 */
const setupMessageListener = (bot) => {
    // grammY uses .on() with context-based handlers
    bot.on('message', async (ctx) => {
        const msg = ctx.message;
        const { chat, text, photo, caption, from: senderInfo, document, location } = msg;
        ...
        if (location) {
            try {
                ...
                logger.info({ event: 'location_saved', userId }, 'User location saved for weather.');

                await commandHandlers.LuminaTyping(currentMessageChatId);
                sendMessage(currentMessageChatId, 'Oke, lokasi sudah disimpan! Aku akan carikan info cuacanya...', {
                    reply_markup: { remove_keyboard: true },
                });
                ...
                if (weatherData) {
                    ...
                    const weatherReminder = weather.getWeatherReminder(weatherData, USER_NAME);
                    const fullResponse = `${weatherString}\n\n${weatherReminder}`;
                    sendMessage(currentMessageChatId, fullResponse);
                } else {
                    sendMessage(
                        currentMessageChatId,
                        `Maaf, ${USER_NAME}, sepertinya Lumina tidak berhasil mendapatkan ` +
                        `data cuaca untuk lokasi tersebut. Coba lagi nanti ya. ${Mood.SAD.emoji}`,
                    );
                }
            } catch (error) {
                logger.error(
                ...
                Sentry.captureException(error);
                sendMessage(currentMessageChatId, 'Aduh, ada kesalahan teknis saat memproses lokasimu. Maaf ya.');
            }
            return;
        }
        ...
        if (document && isFeatureEnabled('ENABLE_DOC_HANDLER')) {
            try {
                ...
                sendMessage(
                    currentMessageChatId,
                    'Oops, Sepertinya ada kesalahan saat saya menganalisis ' +
                    'dokumen, Tuan.',
                );
            }
            return;
        ...
        if (photo && photo.length > 0 && isFeatureEnabled('ENABLE_AI_VISION')) {
            ...
            try {
                ...
                if (visionResult && visionResult.description) {
                    ...
                    logger.warn({ event: 'vision_failed' }, 'VisionAgent did not generate a description.');
                    sendMessage(
                        currentMessageChatId,
        ...
        if (
            !userPromptText ||
            (userPromptText.length === 1 && (isOnlyEmojis(userPromptText) || isOnlyNumbers(userPromptText)))
        ) {
            return;
        }
        ...
        if (!commandHandled) {
            for (const handler of regexHandlers) {
                if (handler.pattern.test(userPromptText)) {
                    ...
                    commandHandled = true;
                    // Exit after the first matching regex handler.
                    break; 
                }
            }
        }
...
module.exports = {
    generateAIResponse,
    /**
     * Initializes and starts the MyLumina bot - the main entry point.
     *
     * This function orchestrates the complete bot startup sequence. Think of it
     * as flipping the "on" switch - it prepares all components before opening
     * the message floodgates.
     *
     * Startup sequence:
     * 1. Set up bot instance reference for command handlers
     * 2. Optimize command routing (O(1) map for prefixed commands)
     * 3. Inject dependencies into AI response generator
     * 4. Restore persisted reminders from previous sessions
     * 5. Initialize TTS schedules if feature enabled
     * 6. Check initial ngambek status
     * 7. Set up time-based modes (sleep, deep talk, etc.)
     * 8. Start cron jobs for scheduled tasks
     * 9. Begin listening for messages
     *
     * The function uses feature flags extensively to allow modular activation
     * of capabilities without code changes.
     *
     * @param {object} bot - The grammY Bot instance with Telegram API token
     * @returns {void}
     * @throws {Error} If critical initialization fails (handled by caller)
     * @example
     * // In index.js - the bootstrap sequence
     * const bot = telegramClient.getBot();
     * initLuminabot(bot);
     * // Bot is now fully operational and listening for messages
     */
    initLuminabot: (bot) => {
        ...
        commandHandlers.commandHandlers.forEach((handler) => {
            ...
            if (match && !patternString.includes('|')) {
                ...
                logger.info(`[Optimizer] Mapped command for O(1) lookup: /${commandKey}`);
            } else {
                ...
                logger.info(`[Optimizer] Added to regex list for iteration: ${patternString}`);
            }
        });
        ...
        if (isFeatureEnabled('ENABLE_CRON_JOBS')) {
            setupCronJobs(bot, updateTimeBasedModes, checkNgambekStatus, Sentry);
        } else {
            logger.warn({ event: 'cron_jobs_disabled' }, 'Cron jobs are disabled by feature flag.');
        }
