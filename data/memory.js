...
const queryCache = new Map(); // Use Map for better performance
...
const getDbInstance = () => {
    if (!initializationPromise) {
        initializationPromise = new Promise((resolve, _reject) => {
            ...
            const db = new Loki(DB_PATH, {
                ...
                autoloadCallback: () => {
                    ...
                    const preferences =
                        db.getCollection('preferences') || db.addCollection('preferences', { unique: ['key'] });
...
const saveLTMMemory = async (key, ltmData) => {
    ...
    if (
        !key ||
        !ltmData ||
        (typeof ltmData.content !== 'string' && typeof ltmData.value !== 'string') ||
        !ltmData.priority
    ) {
            ...
            $or: [
                {
                    ...
                    createdAt: {
                        $lt: new Date(now - LTM_CLEANUP_RULES.highPriority.maxAgeDays * MS_PER_DAY).toISOString(),
                    },
                },
                {
                    priority: {
                    ...
                    createdAt: {
                        $lt: new Date(now - LTM_CLEANUP_RULES.mediumPriority.maxAgeDays * MS_PER_DAY).toISOString(),
                    },
                },
                {
                    priority: {
                    ...
                    createdAt: {
                        $lt: new Date(now - LTM_CLEANUP_RULES.lowPriority.maxAgeDays * MS_PER_DAY).toISOString(),
                    },
                },
            ],
        };
...
const flushHistory = async () => {
    ...
    try {
        ...
        if (currentHistoryCount > FLUSH_THRESHOLD) {
            ...
            const oldMessages = history.chain().simplesort('timestamp').limit(excessCount).data();
...
module.exports = {
    ...
    getAllActiveReminders: async () => {
        ...
        try {
            ...
            return ltm
                .chain()
                .find({ type: 'reminder', expiry: { $gte: now } })
                .data();
        } catch (error) {
