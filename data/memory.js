...
const DB_PATH = process.env.NODE_ENV === 'production' 
  ? path.join(process.cwd(), 'data', 'alya_memory.db') // Path untuk produksi (root project/data/alya_memory.db)
...
const addMessage = async (message) => {
  if (!message.content) {
    console.warn('Attempted to add invalid message: content is missing.', message);
    return;
  }
    ...
    if (currentHistoryCount > MAX_HISTORY_LENGTH) {
      ...
      if (messagesToArchive.length > 0) {
        ...
        await run(
          ...
          [messagesToArchive.map(msg => msg.id).join(',')]
        );
        console.log(`Successfully archived and removed ${messagesToArchive.length} messages from history.`);
      }
    }
    return true;
  } catch (error) {
...
const cleanupOldMessages = async () => {
  try {
    ...
    const deletedHistory = await run('DELETE FROM history WHERE timestamp < ?', [oneWeekAgo]);
    if (deletedHistory.changes > 0) {
      console.log(`Auto-cleanup: Removed ${deletedHistory.changes} old messages from history.`);
    }
    
    // Bersihkan archive lama
    const deletedArchive = await run('DELETE FROM archive WHERE archived_at < ?', [oneWeekAgo]);
    if (deletedArchive.changes > 0) {
      console.log(`Auto-cleanup: Removed ${deletedArchive.changes} old messages from archive.`);
    }
...
// Jadwalkan cleanup mingguan (setiap CLEANUP_INTERVAL)
setInterval(cleanupOldMessages, CLEANUP_INTERVAL);
// Jalankan segera saat startup untuk membersihkan sisa-sisa yang mungkin ada
cleanupOldMessages(); 
...
module.exports = {
  ...
  getInMemoryHistory: async () => {
    const history = await query('SELECT id, role, content, timestamp, chat_id, context FROM history ORDER BY timestamp ASC');
    return history.map(msg => ({
      ...msg,
      context: msg.context ? JSON.parse(msg.context) : {}
    }));
